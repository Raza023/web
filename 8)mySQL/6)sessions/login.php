        ...
        else
        {
            ...
            if($result = mysqli_query($conn,$query))
            {
                if(mysqli_num_rows($result)>=1)   //means email already exists.
                {
                    ...
                    $successMsg .= '<p class="alert alert-success" role="alert"><b>'.mysqli_affected_rows($conn).' row(s) has been inserted. Successfully logged in!</b></p>';
                    $Message = urlencode("Successfully logged in!");
                    header("Location:session.php?Message=".$Message);
                    die;
                }
                else
                {
